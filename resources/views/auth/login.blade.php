...
<!-- Mirrored from ombe.dexignzone.com/bootstrap/xhtml/sign-in.html by HTTrack Website Copier/3.x [XR&CO'2017], Mon, 07 Oct 2024 11:58:55 GMT -->
<head>

    <!-- Title -->
	<title>Recipe Signin Page</title>
		...
		<div class="container py-0">
			<div class="dz-authentication-area">
				...
				<div class="account-section">
					<form class="m-b30" method="POST" action="{{ route('login') }}">
						@csrf
						<div class="mb-4">
							<label class="form-label" for="name">Username</label>
							<div class="input-group input-mini input-lg">
								<input type="email" id="email" name="email" class="form-control @error('email') is-invalid @enderror" value="annie.kuhn@example.com" required>
								@error('email')
						...
						<div class="m-b30">
							...
							<div class="input-group input-mini input-lg">
								<input type="password" id="password" class="form-control dz-password @error('password') is-invalid @enderror" name="password" value="recipe" >
						...
						@if (Route::has('password.request'))
								<p class="form-text">Forgot Password? <a href="{{ route('password.request') }}" class="link ms-2">Reset Password</a></p>
						@endif

					</form>
					<div class="text-center account-footer">
						<p class="text-light">Dont have any account?</p>
						<a href="{{ route('register') }}" class="btn btn-secondary btn-lg btn-thin rounded-xl w-100">CREATE AN ACCOUNT</a>
					</div>
				</div>
			</div>
        </div>
