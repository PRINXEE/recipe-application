	...
	<header class="header header-fixed">
		<div class="header-content">
			...
			<div class="mid-content">
				<h4 class="title">search result for <span id="searchInput">{{$search}}</span></h4>
			</div>
	...
	<main class="page-content space-top">
		<div class="container">
			...
			<div class="search-box">
				<form class="" action="{{ route('search') }}" id="searchForm">
			...
			<!-- Products Area -->
					<div class="dz-custom-swiper search-output">
		        ...
		        @else
							<div class="swiper mySwiper2 dz-tabs-swiper2">
								...
								<div class="row mt-3" id="contentArea">
										<div class="card-body" id="pagination-links">
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    function fetchSearchResults(search, page = 1) {
        $.ajax({
            url: '{{ route("search") }}', // Use the same route
            type: 'GET',
            data: { search: search, page: page }, // Pass the query and the page number
            success: function(response) {
                // Update the search results
                var resultsContainer = $('.search-output');
                resultsContainer.empty(); // Clear previous results

                if (response.recipes.length > 0) {
                  	console.log(response.recipes);
                    var resultList = $('<div class="swiper mySwiper2 dz-tabs-swiper2"></div>');
                    $.each(response.recipes, function(index, recipe) {
                        var averageRating = recipe.average_rating || 0;
                        var recipeImageUrl = recipe.recipe_image;
                        resultList.append(`
													<div class="swiper-wrapper">
															<div class="swiper-slide">
																	<ul class="featured-list">
					                            <li>
					                                <div class="dz-card list">
					                                    <div class="dz-media">
					                                        <a href="javascript:;"><img src="${recipeImageUrl}" alt=""></a>
					                                        <div class="dz-rating"><i class="fa fa-star"></i> ${averageRating.toFixed(1)}</div>
					                                    </div>
					                                    <div class="dz-content">
					                                        <div class="dz-head">
					                                            <h6 class="title"><a href="javascript:;">${recipe.recipe_name}</a></h6>
					                                            <ul class="tag-list">
					                                                <li><a href="javascript:void(0);">${recipe.category_id}</a></li>
					                                                <li><a href="javascript:void(0);">${recipe.difficulty_level}</a></li>
					                                            </ul>
					                                        </div>
					                                        <ul class="dz-meta">
					                                            <li class="dz-price flex-1" style="font-size:15px"><i class="fa fa-clock me-1"></i>${recipe.cooking_time}</li>
					                                            <li>
					                                                <a href="/view-recipe/${recipe.id}" class="btn rounded-xl dz-buy-btn">View</a>
					                                            </li>
					                                        </ul>
					                                    </div>
					                                </div>
    ...
    // Handle the initial search from the search form
    $('#searchForm').on('submit', function(e) {
      e.preventDefault();
      var search = $('input[name="search"]').val();
      fetchSearchResults(search); // Fetch results for the entered query
      // console.log(search);
    ...
    // Handle pagination clicks (delegation to handle dynamic content)
    $(document).on('click', 'contentArea a', function(e) {
      e.preventDefault();
      var search = $('input[name="search"]').val(); // Get the search query
      var page = $(this).attr('href').split('page=')[1]; // Get the page number from the link
      fetchSearchResults(search, page); // Fetch results for the specific page
    });
  });
